...
const getAllCourses = `
    SELECT courses.url, courses.title, courses.timestamp, courses.description, courses.image, courses.exists, sum(courses_parts.length) as "length"
    FROM courses
    ...
    GROUP BY courses.url, courses.title, courses.timestamp, courses.description, courses.image, courses.exists;`

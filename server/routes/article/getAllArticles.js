...
const getAllArticles = `
    SELECT articles.id, url, title, image, text, video, description, timestamp, articles.exists, tags.name as "tags", count(articles_have_comments.articles_id) as "comments",count(articles_comments_have_replies.replies_id) as "replies"
    FROM articles
    ...
    GROUP BY articles.id, url, title, image, text, video, description, timestamp, articles.exists, tags.name
